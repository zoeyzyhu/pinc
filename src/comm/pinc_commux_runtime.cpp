...
bool parse_tcp_init_method(const char* init_method, std::string& host,
  ...
  if (init_method == nullptr || std::strcmp(init_method, "env://") == 0) {
    ...
    const char* env_port = std::getenv("PINC_COMMUX_MASTER_PORT");
    bool explicit_pinc_port = env_port != nullptr && env_port[0] != '\0';
    if (!explicit_pinc_port) env_port = getenv_required("MASTER_PORT");
    if (env_host == nullptr || env_port == nullptr) return false;
    host = env_host;
    port = std::atoi(env_port);
    if (!explicit_pinc_port) port += 101;
    return port > 0;
  }
...
c10d::ReduceOp reduce_op(PINC_CommuxReduceOp op) {
  switch (op) {
    case PINC_COMMUX_SUM:
      return c10d::ReduceOp::SUM;
    case PINC_COMMUX_MIN:
      return c10d::ReduceOp::MIN;
    case PINC_COMMUX_MAX:
...
int collective_error(Runtime* runtime, int local_status) {
  try {
    ...
    return tensor.item<int64_t>() == 0 ? PINC_COMMUX_SUCCESS
                                       : PINC_COMMUX_ERR_RUNTIME;
  } catch (const std::exception& e) {
    set_error("commux collective error check failed: %s", e.what());
    return PINC_COMMUX_ERR_RUNTIME;
  }
}
...
const char* pinc_commux_last_error(void) { return g_last_error; }

int pinc_commux_init(const char* backend, const char* init_method, int rank,
                     int world_size) {
  ...
  if (backend != nullptr && backend[0] != '\0' &&
    ...
    return PINC_COMMUX_ERR_ARG;
  }

  try {
    if (rank < 0) {
      const char* env_rank = getenv_required("RANK");
      if (env_rank == nullptr) return PINC_COMMUX_ERR_ARG;
      rank = std::atoi(env_rank);
    }
    if (world_size < 0) {
      const char* env_world = getenv_required("WORLD_SIZE");
      if (env_world == nullptr) return PINC_COMMUX_ERR_ARG;
...
int pinc_commux_init_env(const char* backend) {
  return pinc_commux_init(backend, "env://", -1, -1);
}

int pinc_commux_finalize(void) {
  std::lock_guard<std::mutex> lock(g_mutex);
  try {
...
int pinc_commux_rank(int* rankp) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (rankp == nullptr) {
    set_error("pinc_commux_rank requires a non-null output pointer");
    return PINC_COMMUX_ERR_ARG;
  }
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  *rankp = runtime->rank;
  return PINC_COMMUX_SUCCESS;
}

int pinc_commux_size(int* sizep) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (sizep == nullptr) {
    set_error("pinc_commux_size requires a non-null output pointer");
    return PINC_COMMUX_ERR_ARG;
  }
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  *sizep = runtime->size;
...
int pinc_commux_allreduce_int64(int64_t value, PINC_CommuxReduceOp op,
                                int64_t* resultp) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (resultp == nullptr) {
    set_error("pinc_commux_allreduce_int64 requires a non-null result");
    return PINC_COMMUX_ERR_ARG;
  }
  if (op < PINC_COMMUX_SUM || op > PINC_COMMUX_MAX) {
...
int pinc_commux_send_int64(int64_t value, int dst, int tag) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  try {
...
int pinc_commux_recv_int64(int src, int tag, int64_t* valuep) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (valuep == nullptr) {
    set_error("pinc_commux_recv_int64 requires a non-null result");
    return PINC_COMMUX_ERR_ARG;
  }
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  try {
...
int pinc_commux_send_bytes(const void* data, int64_t nbytes, int dst, int tag) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  if (nbytes < 0 || (nbytes > 0 && data == nullptr)) {
    set_error("pinc_commux_send_bytes received invalid buffer");
    return PINC_COMMUX_ERR_ARG;
  }
  try {
    auto tensor =
...
int pinc_commux_recv_bytes(void* data, int64_t nbytes, int src, int tag) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  if (nbytes < 0 || (nbytes > 0 && data == nullptr)) {
    set_error("pinc_commux_recv_bytes received invalid buffer");
    return PINC_COMMUX_ERR_ARG;
  }
  try {
    auto tensor =
...
int pinc_commux_file_open(const char* path, int flags, int mode,
                          int64_t* handlep) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (handlep == nullptr || path == nullptr) {
    set_error("pinc_commux_file_open requires path and handle output");
    return PINC_COMMUX_ERR_ARG;
  }
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
...
int pinc_commux_file_close(int64_t handle) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
...
int pinc_commux_file_pwrite(int64_t handle, const void* data, int64_t nbytes,
                            int64_t offset) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
...
int pinc_commux_file_sync(int64_t handle) {
  std::lock_guard<std::mutex> lock(g_mutex);
  Runtime* runtime = runtime_or_error();
  if (runtime == nullptr) return PINC_COMMUX_ERR_RUNTIME;
  auto it = runtime->files.find(handle);
